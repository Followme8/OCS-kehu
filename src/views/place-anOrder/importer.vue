  ...
  <div class="address-container">
    <el-form :inline="true" :model="form" class="demo-form-inline">
      <el-form-item label="类型：">
        <el-select v-model="form.status" placeholder="收件地址">
          <el-option
            v-for="it in muves"
            :key="it.value"
            :label="it.label"
      ...
      <div>
        <el-form-item>
          <el-checkbox v-model="form.myself" :true-label="1" :false-label="0">只查看自己添加</el-checkbox>
        </el-form-item>
      </div>
    </el-form>
    <el-table
  ...
  methods: {
    ...
    async fetchList() {
      try {
        ...
        const res = await api.receivingimporter(creDacs);
        if (res.code === 1) {
          this.list = res.data;
        }
      } catch (error) {
    ...
    postDelete(ids) {
      this.$confirm("是否删除?", "提示", {
        ...
        .then(async () => {
          try {
            const result = await api.receivingimdetele({
              id: ids
            });
            if (result.code === 1) {
              this.fetchList();
